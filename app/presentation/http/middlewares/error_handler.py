...
from app.usecases.errors import BadRequest, Conflict, Forbidden, NotFound, Unauthorized
    ...
    async def __call__(
        ...
        try:
            ...
            if mapped is not None:
                status, code = mapped
                logger.info(
                    "Handled business error %s -> %s", exc.__class__.__name__, code
                )
                response = JSONResponse(
                    status_code=status, content={"code": code, "message": str(exc)}
                )
            ...
            if isinstance(exc, StarletteHTTPException):
                response = JSONResponse(
                    ...
                    content={"code": "HTTP_EXCEPTION", "message": str(exc.detail)},
                )
                await response(scope, receive, send)
                return
